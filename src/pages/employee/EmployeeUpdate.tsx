...
import { GetListRoleService, RoleJson } from '../../services/role.service';
...
const EmployeeSchema = Yup.object().shape({
  ...
  emp_status: Yup.string().required('กรุณาเลือกข้อมูล'),
});
...
const EmployeeUpdate = () => {
  ...
  const [employee, setEmployee] = useState<EmployeeJson>();
  const [roleList, setRoleList] = useState<RoleJson[]>([])

  useMemo(async () => {
    const resEmp = await GetByIdEmployeeService(id);
    const resRole = await GetListRoleService();
    setEmployee(resEmp);
    setRoleList(resRole ? resRole : []);
  }, []);

  const onSubmitEmployee = async (data: UpdateEmployeeJson) => {
    const resp = await UpdateEmployeeService(data);
    if (resp === '200') {
      navigate(-1);
    } 
  };
  ...
  return (
    <>
      <Breadcrumb pageName="เพิ่มสมาชิก" defaultPageName="หน้าหลัก" />
      <Formik
        enableReinitialize
        validationSchema={EmployeeSchema}
        initialValues={{
          emp_id: employee ? employee.emp_id : '',
          emp_username: employee ? employee.emp_username : '',
          emp_password: '',
          emp_new_password:'',
          emp_firstname: employee ? employee.emp_firstname : '',
          emp_lastname: employee ? employee.emp_lastname : '',
          emp_phone: employee ? employee.emp_phone : '',
          emp_email: employee ? employee.emp_email : '',
          emp_status: employee ? employee.emp_status : '',
                ...
                <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
                  <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
                    ...
                    <div className="mb-4">
                      ...
                      {touched.emp_firstname && errors.emp_firstname && (
                        <p className="mt-3 block text-sm font-medium text-danger ">
                          <ErrorMessage name="emp_firstname" />
                        </p>
                      )}
                    </div>
                    <div className="mb-4">
                      ...
                      {touched.emp_lastname && errors.emp_lastname && (
                        <p className="mt-3 block text-sm font-medium text-danger ">
                          <ErrorMessage name="emp_lastname" />
                        </p>
                      )}
                    </div>
                    <div className="mb-4">
                      ...
                      {touched.emp_email && errors.emp_email && (
                        <p className="mt-3 block text-sm font-medium text-danger ">
                          <ErrorMessage name="emp_email" />
                        </p>
                      )}
                    </div>
                    <div className="mb-4">
